...
/**
 ** my code.
 ** store unmerged faces and handle first in the next iteration
 **/
 ...
 ** Sets the mesh to be simplified
 **/
	void InitMergeInput() {this->MergeInput=vtkSurface::New();this->Modified();};
...
//ETX
	...
	// ROI method for this filter
	std::vector<vtkIdType>& GetRoiVertices();
	void SetRoiVertices(std::vector<vtkIdType> vlist);
	void AddRoiVertices(vtkIdType id);
...
private:
	...
	// ROI for this filter
	std::vector<vtkIdType> RoiVertices;

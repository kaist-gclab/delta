...
std::vector<vtkIdType>& vtkWaveletSubdivisionFilter::GetRoiVertices()
{
	return this->RoiVertices;
}

void vtkWaveletSubdivisionFilter::SetRoiVertices(std::vector<vtkIdType> vlist)
{
	this->RoiVertices = vlist;
}

void vtkWaveletSubdivisionFilter::AddRoiVertices(vtkIdType id)
{
	if (std::find(this->RoiVertices.begin(), this->RoiVertices.end(), id) \
		== this->RoiVertices.end()) {
		// id not in rvs, add it
		this->RoiVertices.push_back(id);
	}
}
...
void vtkWaveletSubdivisionFilter::Approximate()
{
	...
	// Lifting dual (pr?iction des coefficients d'ondelette)
...
void vtkWaveletSubdivisionFilter::switchcontour(vtkIdType edge)
{
	...
	else
	{
		...
		/////
		edge2 = this->MergeInput->IsEdge(v2, v3);
		this->MergeInput->GetEdgeFaces(edge2, f1, f2);
		if (f2 < 0) return;
	}
											   ...
											   vtkIdType f2,int incidentfaces)
{
	...
	if (this->vertices[v2].type==CHILD)
	{
		...
		if ((v3!=this->vertices[v2].parent1)&&(v3!=this->vertices[v2].parent2))
		{
			...
			{
				return (0);  //// my code (how does it enter this code...?)
	...
	if (NO1+NO2==2)
	{
		return (0); // my code. i don't know this meaning
	}
...
void vtkWaveletSubdivisionFilter::SolveInverseSubdivision4(vtkIdType initial_face)
{
	...
	////// my code. firstly 1->1 merge for bad faces
	if (this->bad_faces.size() > 0)
	{
		vtkIdType f1;
		for (int i = 0; i < this->bad_faces.size(); i++)
		{
			f1 = this->bad_faces[i];
	...
	//printf("After Regular traverse: remaining_faces=%d\n", remaining_faces);
	...
	//printf("After Second traverse: remaining_faces=%d\n", remaining_faces);
...
fail4:
		...
		///ici interviennent les permutations d'ar?e....
...
fail:
	...
	/////// my code. problematic faces handling
	if (this->remaining_faces > 0)
	{
		for (f1 = 0; f1 < this->MergeInput->GetNumberOfCells(); f1++)
		{
			if (this->faces[f1].hirestype < 0)
			{
				//printf("Not merged face: %d\n", f1);
				this->bad_faces.push_back(f1);
			}
		}
	}
	else
	{
		this->bad_faces.clear();
	}

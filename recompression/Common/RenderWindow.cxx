...
public:
	...
	virtual void OnChar ()
	{
		...
		case 'v':
		{
			...
			vtkSurface *data = (vtkSurface *)this->Window->GetInput();
			Iterator.SetInputData(data);
			/*cout<<"Which Vertex do you want details from ? ";
			cin>>Vertex;
			...
			while (1)
			{
				...
				vtkIdType v1, v2;
				data->GetEdgeVertices(Edge, v1, v2);
				double Point[3];
				if (v1 != Vertex)
					v2 = v1;
				data->GetPoints()->GetPoint(v2, Point);
				cout << "Vertex " << v2 << ", ";
				cout<<"Coordinates : " << Point[0] << " " << Point[1] << " " << Point[2] << endl;
			}
			cout<<endl;
		...
		case 'm':
		{
			// modify vertex position (only for 1st mesh)
			vtkIdType Vertex;
			cout << "Which Vertex do you want to modify? ";
			cin >> Vertex;
			double Point[3];
			this->Window->GetInput()->GetPoints()->GetPoint(Vertex, Point);
			cout << "Coordinates : " << Point[0] << " " << Point[1] << " " << Point[2] << endl;
			int R;
			cout << "Set a radius of neighborhood around the vertex chosen: ";
			cin >> R;

			// ROI insert (initial setting)
			if (this->Window->GetGeometryFlag() == false)
				this->Window->SetGeometryFlag(true);
			...
			// Set new coordinate and propagate to neighbors
			double NewPoint[3];
			cout << "Where do you want to move the vertex? ";
			cin >> NewPoint[0] >> NewPoint[1] >> NewPoint[2];
			double OldPoint[3];
			this->Window->GetInput()->GetPoints()->GetPoint(Vertex, OldPoint);
			double Direction[3];
			Direction[0] = NewPoint[0] - OldPoint[0];
			Direction[1] = NewPoint[1] - OldPoint[1];
			Direction[2] = NewPoint[2] - OldPoint[2];
			cout << "Vertex " << Vertex << " moves to : " << NewPoint[0] << " " << NewPoint[1] << " " << NewPoint[2] << endl;
			this->Window->GetInput()->GetPoints()->SetPoint(Vertex, NewPoint);


			std::vector<vtkIdType> vpool;
			std::vector<vtkIdType> stack;
			vpool.push_back(Vertex);
			stack.push_back(Vertex);

			for (int r = 0; r < R; r++)
			{
				// traverse neighborhood
				std::vector<vtkIdType> neighbors = stack;
				stack.clear();
				cout << "Radius " << r+1 << endl;

				while (!neighbors.empty())
				{
					Iterator.InitTraversal(neighbors.back());
					...
					while (true)
					{
						...
						vtkIdType v1, v2;
						data->GetEdgeVertices(Edge, v1, v2);
						if (v1 != neighbors.back())
							v2 = v1;

						if (std::find(vpool.begin(), vpool.end(), v2) == vpool.end()) {
							// id not in vs, add it
							vpool.push_back(v2);
							stack.push_back(v2);
							...
							double ratio = (R - r) / (double)R;  ///////////// moving ratio (w.r.t. r)

							this->Window->GetInput()->GetPoints()->GetPoint(v2, OldPoint);
							NewPoint[0] = (int)(OldPoint[0] + Direction[0] * ratio);
							NewPoint[1] = (int)(OldPoint[1] + Direction[1] * ratio);
							NewPoint[2] = (int)(OldPoint[2] + Direction[2] * ratio);
							this->Window->GetInput()->GetPoints()->SetPoint(v2, NewPoint);
						}
					}
					neighbors.pop_back();
				}
			...
			// ROI insert (add all vertices)
			for (size_t i = 0; i < vpool.size(); i++)
				this->Window->AddModifiedVertices(vpool[i]);
			...
			//std::vector<vtkIdType> mvs = this->Window->GetModifiedVertices();
			//for (size_t i = 0; i < mvs.size(); i++)
			//{
			//	this->Window->GetInput()->GetPoints()->GetPoint(mvs[i], OldPoint);
			//	NewPoint[0] = OldPoint[0] + Direction[0];
			//	NewPoint[1] = OldPoint[1] + Direction[1];
			//	NewPoint[2] = OldPoint[2] + Direction[2];
			//	this->Window->GetInput()->GetPoints()->SetPoint(mvs[i], NewPoint);
			//}

			//cout << "Vertex " << Vertex << " and neighborhoods is added to ROI " << endl;
			//cout << "ROI : { ";
			//for (size_t i=0; i < mvs.size(); i++)
			//	cout << mvs[i] << ' ';
			//cout << "}" << endl;
...
//	this->MeshActor->GetProperty()->SetSpecular(0.5);
	this->MeshActor->GetProperty()->SetColor(148./255,216./255,246./255);	// SetColor(0.8,0.9,0); yellow color

	this->MeshActor->GetProperty()->SetAmbient(0.2);
...
void RenderWindow::DisplayVerticesColors2(vtkIntArray* Ids, double* color1, double *color2)
{
	double x[4];
	vtkLookupTable* lut = vtkLookupTable::New();

	this->Input->GetPointData()->SetScalars(Ids);
...
void RenderWindow::DisplayVerticesColors3(vtkIntArray* Ids, double* color1, double* color2, double *color3)
{
	double x[4];
	vtkLookupTable* lut = vtkLookupTable::New();
...
bool RenderWindow::GetConnectivityFlag()
{
	return this->isConnectivityChanged;
}

void RenderWindow::SetConnectivityFlag(bool Changed)
{
	this->isConnectivityChanged = Changed;
}

bool RenderWindow::GetGeometryFlag()
{
	return this->isGeometryChanged;
}

void RenderWindow::SetGeometryFlag(bool Changed)
{
	this->isGeometryChanged = Changed;
}

std::vector<vtkIdType> RenderWindow::GetModifiedVertices()
{
	return this->ModifiedVertices;
}

void RenderWindow::AddModifiedVertices(vtkIdType id)
{
	if (std::find(this->ModifiedVertices.begin(), this->ModifiedVertices.end(), id) \
		== this->ModifiedVertices.end()) {
		// id not in mvs, add it
		this->ModifiedVertices.push_back(id);
	}
}

void RenderWindow::ClearModifiedVertices()
{
...
//	this->renWin->GetInteractor()->Delete();
	...
	this->ModifiedVertices = std::vector<vtkIdType>();
}
